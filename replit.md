...
## Overview
A web-based employee shift management application for scheduling, tracking, and managing employee work shifts. Built with React + Express + PostgreSQL. Features three-tier authentication (Admin/Manager/Employee) with access codes and SteepIn for time tracking.
...
## Project Structure
- `client/src/pages/` - Dashboard, Schedule, Employees, Login, Kiosk (SteepIn) pages
...
- **SteepIn**: No login required, for clock-in/out and break tracking
...
- `POST /api/kiosk/action` - Record clock-in/out/break-start/break-end
...
- SteepIn for clock-in/out and break tracking
