...
export function ShiftFormDialog({
  ...
  const allowProximityRef = useRef(false);
  ...
  const mutation = useMutation({
    mutationFn: async (values: ShiftFormValues & { allowProximity?: boolean }) => {
      if (isEditing) {
    ...
    onSuccess: () => {
      ...
      pendingData.current = null;
      allowProximityRef.current = false;
      onOpenChange(false);
      form.reset();
    },
    onError: (error: Error) => {
      // Try to parse JSON from error body
      ...
      let errorType = parsed?.type;
      let errorMessage = parsed?.message ?? error.message;
      if (!parsed) {
        const msgMatch = error.message.match(/"message"\s*:\s*"((?:[^"\\]|\\.)*)"/);
        if (msgMatch) errorMessage = msgMatch[1];
        else errorMessage = error.message.replace(/^\d+:\s*/, "");
        const typeMatch = error.message.match(/"type"\s*:\s*"([^"]+)"/);
        if (typeMatch) errorType = typeMatch[1];
      }

      if (errorType === "proximity-warning") {
        const nearby = parsed?.nearbyShift;
        const nearbyLabel = nearby ? ` (${nearby.startTime}–${nearby.endTime})` : "";
        const savedValues = pendingData.current || form.getValues();
        setShiftWarning({
          title: "Shifts Are Very Close Together",
          description: `The new shift is less than 20 minutes away from an existing shift${nearbyLabel}. Consider merging both into a single, longer shift instead. Do you still want to create this shift separately?`,
          actions: [
            {
              ...
              onClick: () => {
                setShiftWarning(null);
                allowProximityRef.current = true;
                runMutate(savedValues);
                pendingData.current = null;
              },
            },
            {
  ...
  const runMutate = (values: ShiftFormValues) => {
    ...
    mutation.mutate({
      ...
      allowProximity: allowProximityRef.current,
    });
  };
