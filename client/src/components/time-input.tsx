...
function ClockPickerDialog({ open, onOpenChange, value, onChange }: ClockPickerDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-[320px] p-0 overflow-hidden rounded-2xl border-0 gap-0" onPointerDownOutside={(e) => e.preventDefault()} onInteractOutside={(e) => e.preventDefault()}>
  ...
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1">
        <TimeInput value={startValue} onChange={handleStartChange} data-testid={startTestId} />
      </div>
      <span className="text-sm text-muted-foreground font-medium select-none shrink-0">to</span>
      <div className="flex-1 relative">
        ...
        >
          {endValue && /^\d{2}:\d{2}$/.test(endValue) ? endValue : <span className="text-muted-foreground">HH:MM</span>}
        </button>
        <ClockPickerDialog
          open={endPickerOpen}
